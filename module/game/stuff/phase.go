...
func (p *Phase) Data() map[int][]*turn {
	return p.phases
}

func (p *Phase) Init() {
	p.currentPhaseId = enum.NightPhase
	p.phases = make(map[int][]*turn)
}
...
func (p *Phase) AddTurn(phaseId int, timeout time.Duration, role itf.IRole, playerIds []int) bool {
...
func (p *Phase) RemoveTurn(phaseId int, roleName string) bool {
...
func (p *Phase) AddPlayer(phaseId int, turnIndex int, playerIds ...int) bool {
